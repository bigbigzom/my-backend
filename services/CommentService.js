...
export class CommentService {
  ...
  /** 发布评论（主账号策略）v6.3：默认gaia=false避免代理IP下WBI签名超时 */
  async addComment({ accountId, oid, message, type = 1, mode = COMMENT_MODE.NORMAL, gaia = false }) {
    ...
    const result = await commentApi.add({ oid, message, type, mode, gaia });
