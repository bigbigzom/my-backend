...
export const ACCOUNT_TYPE = {
  VIDEO_PUBLISHER: 'video_publisher',  // 视频发布账号（最终发布引流视频）
  COMMENT_ACCOUNT: 'comment_account',    // 评论区账号（只用于评论引流）
};
    ...
    try {
      ...
      for (const bvid of videosToBrowse) {
        try {
          ...
          // 随机互动（基于阶段配置的概率）
          if (Math.random() < dailyActions.like / dailyActions.browse) {
            await this.behaviorSimulator.likeVideo(bvid);
            results.like++;
            await this._humanDelay(1, 3);
          }
          if (Math.random() < dailyActions.coin / dailyActions.browse) {
            await this.behaviorSimulator.coinVideo(bvid, 1);
            results.coin++;
            await this._humanDelay(1, 3);
          }
          if (Math.random() < dailyActions.favorite / dailyActions.browse) {
          ...
          if (Math.random() < dailyActions.comment / dailyActions.browse) {
