...
export class AccountCultivator {
  ...
  async runDailyCultivation(options = {}) {
    ...
    // 改进2：视频发布者账号不再参与评论区发布（养号也只做观看/点赞/投币等自我账号行为）
    if (this.accountType === ACCOUNT_TYPE.VIDEO_PUBLISHER) {
      const skipped = ['comment', 'danmaku'];
      for (const k of skipped) {
        if (dailyActions[k] > 0) {
          this.getLogger(`[Cultivator] 视频发布者账号跳过 ${k} 动作（不参与评论区发布）`);
          dailyActions[k] = 0;
        }
      }
    }
  ...
  async _simulateVideoWatch(bvid, maxWatchMin) {
    ...
    try {
      // BUG修复(B9)：watchVideo 第二参应为 opts 对象；传入数字会被当作 opts.watchRatio=undefined 而失效
      // 用 watchRatio 约束观看比例，minWatchSec 兜底最低观看秒数
      await this.behaviorSimulator.watchVideo(bvid, {
        watchRatio: Math.min(0.7, Math.max(0.2, watchMin / 60)),
        minWatchSec: Math.round(Math.min(watchMin * 60, 300)),
      });
    } catch (e) {
      // 忽略
    }
  ...
  async _simulateDanmaku(bvid) {
    ...
    // BUG修复(B10)：原实现仅打日志从不真正发送。真实弹幕接口 POST /x/v2/dm/post，
    // 需要先取视频 cid 作为 oid（注意弹幕的 oid 是 cid 而非 aid），progress 为弹幕出现时间(ms)。
    try {
      const videoInfo = await this.client.get('https://api.bilibili.com/x/web-interface/view', { bvid });
      const d = videoInfo && videoInfo.data && videoInfo.data.data;
      if (d && d.cid) {
        await this.client.postForm('https://api.bilibili.com/x/v2/dm/post', {
          type: '1',
          oid: String(d.cid),
          msg: content,
          progress: String(Math.floor(1000 + Math.random() * Math.max(1000, (d.duration || 60) * 1000 * 0.8))),
          color: '16777215',
          fontsize: '25',
          pool: '0',
          mode: '1',
          rnd: String(Math.floor(Math.random() * 100000000)),
          csrf: this.client.csrf,
        });
      }
    } catch (e) {
      this.getLogger(`[Cultivator] 发弹幕失败(忽略): ${e.message}`);
    }
  }
  ...
  async _simulateComment(bvid) {
    ...
    try {
      // 需要先获取oid（aid）
      const videoInfo = await this.client.get('https://api.bilibili.com/x/web-interface/view', { bvid });
      if (videoInfo && videoInfo.data && videoInfo.data.data && videoInfo.data.data.aid) {
        // BUG修复(B1)：原代码调用不存在的 commentApi.addComment()，导致养号评论永远静默失败
        // 正确方法是 CommentAPI.add()，并带上 GAIA 参数与 statistics 平台安全机制统计字段
        await this.commentApi.add({
          oid: videoInfo.data.data.aid,
          message: content,
          type: 1,
        });
      }
    } catch (e) {
      this.getLogger(`[Cultivator] 发评论失败(忽略): ${e.message}`);
    }
  }
