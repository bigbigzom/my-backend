...
import {
  ensureActiveWindow, ensureNextRefreshTime, onRefreshCompleted, getAccountUsagePolicy,
} from './CookieUseScheduler.js';
...
export class AccountManagerV2 extends EventEmitter {
  ...
  add(accountData) {
    ...
    this.ensureSchedulingFields(account);
    this.accounts.set(account.id, account);
    this._save();
  ...
  // ============================================================
  // Cookie 延迟使用调度（v3.1：温号初始化 + 错峰窗口 + 错峰刷新）
  // ============================================================
  /**
   * 确保账号具备 Cookie 延迟使用调度字段
   * - 新导入/新刷新账号：温号期从当下开始（冷启动冷静期）
   * - 老账号迁移：未设置时默认 warmUpDisabled=true（不强制新账号也冷静）
   * - 独立活跃窗口 + 独立下次刷新时刻
   */
  ensureSchedulingFields(account) {
    if (!account.warmUpStartedAt) {
      // 老账号（createdAt 早于本次升级）默认关闭温号，避免存量账号突然不可用
      const isLegacy = account.createdAt &&
        new Date(account.createdAt).getTime() < new Date('2026-08-30').getTime();
      if (isLegacy) {
        account.warmUpDisabled = true;
      } else {
        account.warmUpStartedAt = new Date().toISOString();
      }
    }
    ensureActiveWindow(account);
    ensureNextRefreshTime(account);
    return account;
  }

  /**
   * 获取账号 Cookie 使用策略（温号等级 + 活跃窗口）
   */
  getUsagePolicy(id) {
    const account = this.accounts.get(id);
    if (!account) return null;
    this.ensureSchedulingFields(account);
    const policy = getAccountUsagePolicy(account);
    this._save();
    return { id, uid: account.uid, ...policy };
  }
   ...
   */
  getAllUsagePolicies() {
    return this.getAll().map(a => this.getUsagePolicy(a.id));
  }
  ...
  getRefreshDue() {
    return this.getAll().filter(a => {
      if (!a.canRefresh || !a.isActive) return false;
      this.ensureSchedulingFields(a);
      return new Date() >= new Date(a.nextRefreshAt);
    });
  }

  /**
   * 错峰刷新：逐个刷新到期账号（账号间随机间隔，模拟真人续期节奏）
   */
  async refreshDueAccounts(options = {}) {
    const due = this.getRefreshDue();
    const results = [];
    let success = 0, failed = 0, skipped = 0;
    for (const account of due) {
      const r = await this.refreshAccount(account.id, options);
      results.push({ id: account.id, uid: account.uid, ...r });
      if (r.success && !r.skipped) success++;
      else if (r.skipped) skipped++;
      else failed++;
      // 错峰：账号间随机 20~40 分钟间隔，绝不齐刷
      await new Promise(r => setTimeout(r, (20 + Math.random() * 20) * 60000));
    }
    return { total: due.length, success, failed, skipped, results };
  }
    ...
    accounts.forEach(data => {
      try {
        ...
        const account = new Account({
          ...
          // BUG修复(B3)：透传完整设备环境与指纹，保证后端操作/续期与本地登录设备一致。
          // 本地登录服务同步时携带 deviceProfile/deviceEnv（等价字段），此前被丢弃。
          deviceProfile: data.deviceProfile || data.deviceEnv || null,
