 ...
 * - primaryProxy: 账号主用IP（独立运营：固定常用IP，模拟真实用户所在地）
...
class AccountManager {
  ...
  migrate() {
    for (const a of this.accounts) {
      ...
      // v2.3 独立运营：发布历史（记录每视频评论数，用于单视频上限）
      if (!a.publishHistory) a.publishHistory = [];
      // v2.3 独立运营：社交关联标记（是否与发布账号有关联）
      if (a.socialSeparation === undefined) a.socialSeparation = true; // 默认启用社交隔离
      // v2.3 独立运营：IP角色（publisher=发布视频账号 / commenter=评论内容互动账号），用于IP池分离
  ...
  pickRandomMain(opts = {}) {
    ...
    const candidates = this.getAvailable().filter(a => {
      // 改进2：视频发布者账号不再参与评论区发布
      if (a.ipRole === 'publisher') return false;
    ...
    const weights = candidates.map(a => {
      ...
      const riskPenalty = Math.max(0, (a.riskScore || 0) / 200);  // 运营关联度分降权
  ...
  /**
   * 新号冷静期判断（新注册账号不参与策略，降低"新号+内容互动"高危组合）
   */
   ...
   * 5.社交图谱 6.账号基建 7.网络流量 8.独立运营措施执行度
   */
  computeRiskScore(id) {
    const account = this.getById(id);
    if (!account) return 0;
    ...
    if (nurtureActions < 3 && (account.todayPublished || 0) > 0) {
      breakdown.concentration += 5; // 日常运营不足但有发布 = 高集中度特征
    }
  ...
  // ============================================================
  // v2.3 独立运营：IP池分离（发布者IP vs 评论者IP）
  // ============================================================
  /**
