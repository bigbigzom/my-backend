...
import { generateFingerprint, injectFingerprint, getOrCreateFingerprint, applyFingerprint } from '../utils/browser-fingerprint.js';
import { simulateType, simulateClick, simulateMouseMove, simulateScroll, getBehaviorProfileConfig } from '../utils/human-behavior.js';
...
export class BilibiliLogin {
  ...
  async launch(accountKey = 'default') {
    ...
    const launchArgs = [
      ...
      '--lang=zh-CN',
      // v2.2 WebRTC 防真IP泄漏（HTTP代理下防止暴露真实IP）
      '--disable-webrtc',
    ...
    // ★ 浏览器指纹随机化（v2.2：账号绑定稳定指纹画像，同账号复用同指纹）
    if (this.useFingerprint) {
      console.log('[BilibiliLogin] 获取账号指纹画像（v2.2 画像化，同账号稳定复用）...');
      // 代理IP城市 → 指纹时区地理一致
      const proxyCity = this.proxy && this.proxy.city ? this.proxy.city : null;
      const proxyIp = this.proxy ? this.proxy.ip : null;
      this.fingerprint = getOrCreateFingerprint(accountKey, { proxyCity, proxyIp });
      await injectFingerprint(this.page, this.fingerprint, { proxyIp });
      ...
      console.log(`[BilibiliLogin] 指纹ID=${this.fingerprint.fingerprintId}（绑定账号: ${accountKey}）`);
    } else {
      // 不使用指纹随机化时，基础反检测
    ...
    // ★ 登录前验证代理出口IP（确保是中国IP，防止代理失效被B站平台安全机制）
  ...
  async loginByPhoneCode(phone, options = {}) {
    ...
    try {
      ...
      await simulateType(this.page, 'input[type="tel"], input[placeholder*="手机"], input[placeholder*="手机号"]', phone, null, { ime: false });
  ...
  async loginByPassword(account, password, options = {}) {
    ...
    try {
      ...
      await simulateType(this.page, 'input[placeholder*="账号"], input[placeholder*="手机"], input[placeholder*="邮箱"], input[type="text"]', account, null, { ime: false });
      await new Promise(r => setTimeout(r, randomDelay(500, 1000)));
      ...
      await simulateType(this.page, 'input[type="password"]', password, null, { ime: false });
      await new Promise(r => setTimeout(r, randomDelay(500, 1000)));
  ...
  async close(loginSuccess = true) {
    ...
    if (this.browser) {
      ...
      this.browser = null;
      this.page = null;
      // 如果登录失败且使用了代理，标记该代理失败（可能是代理被B站账号受限）
