...
export class BiliClient {
  ...
  constructor(options = {}) {
    ...
    // v1.4（HAR对齐）：若设备环境中采集到 buvid_fp/buvid4/buvid3 而 cookie 中缺失，
    // 自动补入 cookie 串——B站风控依赖这些设备指纹，保证后端请求与登录浏览器环境一致。
    if (this.deviceProfile) {
      const buvidCookies = [];
      for (const k of ['buvid_fp', 'buvid4', 'buvid3']) {
        const v = this.deviceProfile[k];
        if (v && this.cookieStr.indexOf(`${k}=`) === -1) buvidCookies.push(`${k}=${v}`);
      }
      if (buvidCookies.length > 0) {
        this.cookieStr = this.cookieStr ? this.cookieStr + '; ' + buvidCookies.join('; ') : buvidCookies.join('; ');
      }
    }
    ...
    this.mid = midMatch ? midMatch[1].trim() : '';
  ...
  /** 获取 undici dispatcher（代理绑定时返回 ProxyAgent，实现账号注册IP出口一致性） */
  ...
  async _request(method, url, options = {}) {
    ...
    // v6.3：支持相对路径（自动补全 https://api.bilibili.com）
    if (url.startsWith('/')) {
    ...
    if (needWbiSign) {
      ...
      // B8修复：使用 signAndGetQuery —— 返回的 query 与签名时哈希的字符串逐字节一致，
      // 直接原样作为 URL query，避免调用方重新编码导致含 !'()* 参数时签名校验失败
      const { query } = this._wbiSigner.signAndGetQuery(finalParams);
      wbiQuery = query;
    }
    ...
    for (let attempt = 0; attempt <= this.retryCount; attempt++) {
      try {
        ...
        const res = await fetch(fullUrl, {
          ...
          // 改进1：账号绑定注册IP（经 undici ProxyAgent），实现 IP 粘性
          dispatcher: await this._getDispatcher(),
        });
        clearTimeout(timer);
