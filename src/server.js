...
import { startProxyPool, getProxyPoolStats, refreshProxyPool, getProxy, acquireProxy, markProxyFailed, isProxyReady, getAvailableProxies } from './utils/proxy-pool.js';
import {
...
// v1.5.4：获取可用中国IP列表（供本地登录服务远程拉取，本地不再自己维护代理池）
app.get('/api/proxy/available', (req, res) => {
  const limit = parseInt(req.query.limit) || 0;
  const list = getAvailableProxies(limit);
  res.json({ code: 0, data: { count: list.length, proxies: list, updatedAt: new Date().toISOString() } });
});
// 手动触发代理池刷新
