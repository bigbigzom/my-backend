...
import { AccountManagerV2 } from './accounts-v2/index.js';
...
function createBiliClientForAccount(accountId) {
  const acc = AccountManagerV2Inst.get(accountId);
  if (!acc) return null;
  // 使用账号的完整设备环境（本地登录时采集），确保后端操作与登录时环境一致，降低风控
...
const server = app.listen(PORT, () => {
  ...
  // v2定时：每天凌晨3点批量刷新
  const next3am = new Date(); next3am.setHours(27, 0, 0, 0);
  setTimeout(() => {
    setInterval(async () => {
      try {
        const r = await AccountManagerV2Inst.refreshAllNeedsRefresh();
        console.log(`[v2] 批量刷新: 成功${r.success}, 失败${r.failed}, 跳过${r.skipped}`);
      } catch (e) { console.error('[v2] 批量刷新失败:', e.message); }
    }, 24 * 60 * 60 * 1000);
  }, next3am - Date.now());
});
