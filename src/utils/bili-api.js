...
import { getProxy, waitForProxy, markProxyFailed, getProxyPoolStats } from './proxy-pool.js';
...
// 请求头（完全对齐HAR数据包）
// ============================================================
function getBiliHeaders(account, bv) {
  return {
  ...
  while (attempt <= config.maxRetries) {
    ...
    try {
      ...
      if (useProxy) {
        let proxy = getProxy();
        if (!proxy) {
          console.log(`[BiliAPI] 代理池为空，等待代理... (尝试 ${attempt})`);
      ...
      if (useProxy && usedProxy) {
        if (data.code === -352 || data.code === -403 || 
          ...
          console.warn(`[BiliAPI] 代理 ${usedProxy} 被B站拦截 (code=${data.code})，标记失败并重试`);
