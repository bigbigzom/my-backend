...
import { WbiSigner } from '../bili-api/WbiSigner.js';
// B5修复：复用新版面向对象 WbiSigner 实现 WBI 签名（旧版 useWbi 参数此前从未真正实现）
...
async function biliFetch({ url, method = 'GET', headers = {}, body = null, account = null, bv = null, useWbi = false }) {
  ...
  while (attempt <= config.maxRetries) {
    ...
    try {
      ...
      // 检测代理是否被B站拦截（海外IP限制、平台安全机制）
      if (useProxy && usedProxy) {
        if (data.code === -352 || data.code === -403 || 
            (data.message && (data.message.includes('海外') || data.message.includes('境外') || data.message.includes('平台安全机制')))) {
          console.warn(`[BiliAPI] 代理 ${usedProxy} 被B站拦截 (code=${data.code})，标记账号受限`);
          markProxyBlocked(usedProxy);  // v2.2: 被B站账号受限的IP立即剔除
...
export async function getReplyList({ bv, oid, account, mode = 3, offset = '' }) {
  ...
  // B5修复：WBI 签名在 biliFetch(useWbi=true) 内自动完成（w_rid + wts）
