 ...
 * @param {Object} opts { proxyIp: 代理出口IP（用于WebRTC模拟） }
 */
export async function injectFingerprint(page, fingerprint, opts = {}) {
  const fp = fingerprint;
  // WebRTC 模拟IP：用代理出口IP（无代理则用随机中国IP段）
  ...
  await page.evaluateOnNewDocument((f, wip) => {
    ...
    // ========================================================
    // 5. 字体列表指纹（canvas 测量字体，返回模拟的字体列表）
    // ========================================================
    try {
      Object.defineProperty(document, 'fonts', {
        get: () => ({
          check: (font) => true,  // 所有字体都存在（模拟）
    ...
    // ========================================================
    // 10. WebRTC 网络一致性（模拟与网络出口一致的本地/对等IP）
    // ========================================================
    try {
      const webrtcCandidateHack = () => {
        // 覆盖 RTCPeerConnection，模拟 ICE candidate IP
