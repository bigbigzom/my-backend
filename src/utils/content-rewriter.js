 ...
 * 2. 内容互动词变体化：微信/QQ/加群 等强内容互动词 → 谐音/符号/拼音/私信引导变体
 * 3. 对话剧本生成：主号内容互动 + 次级号提问/晒单/赞同 的真实对话场
 * 4. 人设差异化：按账号人设选择语气模板
 *
 * 设计原则：
 * - 每条评论内容互动词最多1个变体（防连续多条同变体指纹）
 * - 不同账号用不同变体
...
// ============================================================
// v2.3 独立运营：弱相关文案池（次级号评论不夸UP主，做内容相关独立视角/生活化闲聊）
// 避免"过度配合UP主=辅助账号"的信号
// ============================================================
const WEAK_RELEVANCE_COPY = [
  // 内容相关独立视角（不夸UP主，只谈内容本身）
  '这个知识点之前在别的地方也看到过',
  '说到这个，我之前也遇到过类似的情况',
  '这个方法我试过，确实有用',
  '原来还可以这样操作，涨知识了',
  '这个角度挺有意思的，之前没想过',
  // 生活化闲聊（与视频弱相关，像真实用户随手评论）
  '刚好在吃饭的时候刷到这个',
  '下班回家刷到，刚好需要这个',
  '今天摸鱼看了好几个类似的',
  '这个点还有人在看吗',
  '睡前刷到，先码住明天看',
  // 独立提问（不配合UP主，像真实用户的疑问）
  '有没有人知道这个具体怎么操作的',
  '这个对新手友好吗',
  '需要什么基础才能弄这个',
  '这个成本大概多少啊',
...
/**
 * 生成弱相关次级号评论（不夸UP主，降低"辅助账号"的嫌疑）
 * @param {Object} opts { keyword, persona }
 */
export function generateWeakRelevanceCopy(opts = {}) {
  const keyword = opts.keyword || '';
  let text = pick(WEAK_RELEVANCE_COPY);
  // 30%概率替换为含关键词的弱相关评论
  if (keyword && chance(0.3)) {
    const kwTemplates = [
      `说到${keyword}，我之前也了解过一点`,
      `${keyword}这个东西水挺深的`,
      `有没有人一起研究${keyword}的`,
      `${keyword}最近好像挺火的`,
    ];
    text = pick(kwTemplates);
  }
  return semanticRewrite(text);
}
// ============================================================
// 工具函数
// ============================================================
...
/**
 * 生成主号内容互动文案（语义重排 + 内容互动词变体）
 * @param {String} base 用户提供的基础内容互动文案（可含 {action} 占位）
 * @param {Object} opts { variantIndex, keyword }
 */
export function generateMainCopy(base, opts = {}) {
  if (!base) return '';
  let text = base.trim();
  // 若用户文案含内容互动意图词但无占位，自动附加引导
...
export function generateSubCopy(persona = 'neutral', opts = {}) {
  const keyword = opts.keyword || '';
  const kw = keyword || '这个';
  // v2.3 独立运营：50%概率使用弱相关文案（不夸UP主，降低"辅助账号"的嫌疑）
  if (opts.weakRelevance !== false && chance(0.5)) {
    return generateWeakRelevanceCopy({ keyword, persona });
  }
  switch (persona) {
    case 'questioner':
