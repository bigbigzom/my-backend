 ...
 * 核心内容互动策略引擎 v2.2
 *
 * 核心诉求：主号发布关键内容互动信息 → 次级号楼中楼跟随+点赞 → 推上评论区最热置顶
 *
 * v2.2 重大升级：
 ...
 * 4. 智能选主号：健康度加权随机
 * 5. 内容互动词变体化 + 语义重排（反文案指纹）
 * 6. 人设差异化：次级号按人设生成不同语气
 * 7. 失败分级处理：-352平台安全机制→冷却换号 / -101未登录→标记 / -102账号受限→标记banned / 网络→重试
 */
import AccountManager, { PERSONAS } from '../accounts/account-manager.js';
import { maybeGateByWarmUp } from '../accounts-v2/CookieUseScheduler.js';
...
const strategyConfig = {
  ...
  variantPerAccount: true,      // 不同账号用不同内容互动词变体
};
/**
 * 更新策略参数（前端配置）
 */
...
function classifyError(result) {
  ...
  switch (code) {
    case -352: return { level: 'cooldown', code, message: '触发平台安全校验/需验证码，冷却该账号' };
    case -102: case -103: return { level: 'banned', code, message: '账号被账号受限' };
    case -101: return { level: 'relogin', code, message: '登录态失效，需重新登录' };
    case -403: case -412: case -111: return { level: 'proxy_blocked', code, message: '请求被拦截（可能网络出口受限）' };
 ...
 * @param {Object} params {
 *   bv: 视频BV号,
 *   mainCopy: 主号内容互动文案（可含{action}占位）,
...
export async function executeStrategy(params = {}) {
  ...
  if (!mainAccount) return { code: -1, message: '无可用主账号（可能都在冷却/新号冷静期/被账号受限）' };
  // 3. 选次级号
  let subAccounts;
  ...
  // 4.5 独立运营：单视频本系统评论上限检查（超限则只点赞不评论）
  ...
  let mainResult = null;
  if (mainText) {
    // v3.1 温号门控：发布评论要求账号至少处于"全开放期"
    const gate = maybeGateByWarmUp(mainAccount, 'comment');
    if (!gate.allowed) {
      return { code: -1, message: `主账号发布被温号期拦截: ${gate.reason}`, warmUpBlocked: true };
    }
  ...
  if (!onlyLike && mainResult && mainResult.rpid) {
    // v2.3 独立运营：每个次级号随机行为模式（避免固定"先回复后点赞"流程）
...
taskQueue.register('strategy_reply', async (data) => {
  ...
  // v3.1 温号门控
  const gate = maybeGateByWarmUp(account, 'comment');
  if (!gate.allowed) throw new Error(`温号期拦截: ${gate.reason}`);
  // 令牌桶控制
...
taskQueue.register('strategy_like', async (data) => {
  ...
  if (account.status !== 'normal') throw new Error(`账号状态异常: ${account.status}`);
  // v3.1 温号门控（点赞只需低风险等级）
  const gate = maybeGateByWarmUp(account, 'like');
  if (!gate.allowed) throw new Error(`温号期拦截: ${gate.reason}`);
  if (!globalBucket.tryTake(1)) throw new Error('全局速率限制，稍后重试');
  // v2.3 独立运营：点赞前先"浏览"评论区（模拟"看到→觉得好→点赞"的自然行为）
...
taskQueue.register('nurture_task', async (data) => {
  ...
  // 用账号自己的代理发普通评论（非内容互动）—— 简化：仅记录日志，实际可在浏览器中执行
